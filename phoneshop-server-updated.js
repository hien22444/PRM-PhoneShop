...
app.post("/api/orders/from-cart", (req, res) => {
  ...
  // Lưu đơn hàng
  db.orders.push(newOrder);
  console.log(`📦 ORDER FROM CART: ${newOrder.id} for user ${userId} (${cart.items.length} items)`);
  ...
  if (cartIndex !== -1) {
    const cartItemCount = db.carts[cartIndex].items.length;
    db.carts.splice(cartIndex, 1);
    console.log(`🗑️ CART CLEARED: Removed ${cartItemCount} items from user ${userId} cart after order from cart`);
  }
...
app.get("/api/orders/:userId", (req, res) => {
  ...
  res.json({
    ...
    totalPages: Math.ceil(userOrders.length / parseInt(size)),
